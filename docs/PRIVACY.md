...
A second, narrower case was closed in September 2026. On Linux, when `xdotool` stalled or was missing, the log recorded the whole command the keyboard had handed it, and for typed text that command is the text; on macOS a chorded key the layout had no keycode for was logged by character. Both now record nothing about the key, and the first launch of the fixed build on those platforms deletes the old log files once, as the first fix did. Windows logs never held either record.
...
**The Windows installer asks the same question**, on its own page. That page shows you the whole message, field by field, with example numbers in it, and says what the numbers are for; the box on it is **ticked**, so clicking straight past accepts. Unticking it, which is one click, declines and changes nothing. Ticking it is the same consent as turning the toggle on, and it is applied the first time you launch the keyboard. Whichever you choose, the switch in Settings still governs it afterwards, and you can delete everything you have sent. A silent install, which is what an automatic update runs, never shows the page and never changes what you already chose.
