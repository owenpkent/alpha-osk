...
### 7.4 Dependency lockfiles and SBOM posture

Both build pipelines (`build/windows/build.py::freeze_lockfile` and `build/linux/build.py::freeze_lockfile`) run `pip freeze --all` against the build venv on every build and write the result to `release/Alpha-OSK-Setup-{version}-requirements.lock.txt` (Windows) or `release/Alpha-OSK-{version}-linux-requirements.lock.txt` (Linux). The lockfile is uploaded as a release asset alongside the installer / AppImage. A reviewer or future maintainer can `pip install -r <lockfile>` into a fresh venv to recreate the exact dependency tree PyInstaller bundled, which until 1.1.0 was unanswerable — no lockfile means "what version of urllib3 shipped in 1.0.16?" had no definitive answer.

This is **not** a CycloneDX or SPDX SBOM. There are no license fields, no package URLs (purl), no signed statements. US Executive Order 14028 and most enterprise procurement processes ask for a structured SBOM, not a `pip freeze` dump. The plaintext lockfile is the cheap floor; the proper SBOM upgrade path is `pip install cyclonedx-bom` in the build venv + `cyclonedx-py environment` emitting `release/Alpha-OSK-Setup-{version}-sbom.cyclonedx.json`, plus `@cyclonedx/cyclonedx-npm` for the worker (`backend/cf-worker/`), plus `osv-scanner` as a CI job reading the lockfile to flag transitive CVEs on every PR. The upgrade is one engineering session; it has not been done because no current user has asked for it. If Alpha-OSK ever lands in a hospital procurement review or any federal supply-chain audit, the SBOM is the first thing the reviewer asks for, and the lockfile gives an honest base to build it from.

`backend/cf-worker/package-lock.json` is checked in alongside `package.json` so Wrangler / TypeScript / `@cloudflare/workers-types` versions are deterministic between local and CI (with `node_modules/` in `.gitignore` — the lockfile is the source of truth, materialised by `npm install` or `npm ci`). Generating that lockfile incidentally surfaced four dev-only CVEs flowing through Wrangler 3.x (three moderate `esbuild`, one high `undici`) — they affect only the local `wrangler dev --local` dev server, not the deployed worker on Cloudflare's edge runtime where those packages don't exist. Upgrading to Wrangler 4.x fixes them but is a breaking change; deferred. The surfacing is exactly the point of having a lockfile.
...
6. **Structured SBOM at release time.** The plaintext lockfile (§7.4) is the cheap floor for build reproducibility but not a CycloneDX or SPDX SBOM. Enterprise procurement, hospital review, and federal supply-chain audit per Executive Order 14028 all expect a structured SBOM. Upgrade is roughly one engineering session (cyclonedx-bom + a CI osv-scanner job); deferred until a real requester appears.
