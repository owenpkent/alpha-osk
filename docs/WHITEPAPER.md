...
**Document revision:** 2.1 (16 September 2026), describing Alpha-OSK 1.3.0; evaluation claims clarified without rerunning benchmarks
...
On held-out data from a crowdsourced corpus of AAC-like communications, the measured engine configuration avoids 49.1% and 50.4% of keystrokes under an ideal prediction-selection policy, offering the correct next word before any of its letters are typed about three times in ten, at a median 2.9 ms per prediction call. On these clean-input benchmarks, including the spatial source does not improve KSR, and restoring the character model's word candidates lowers KSR while increasing median latency more than fivefold. Under a synthetic adjacent-key slip condition, the prefix beam improves KSR by eight to nine percentage points over the previous whole-word fuzzy source. These are simulation results, not measurements of user effort or correction accuracy. This paper describes the architecture, the prediction stack, the accessibility-driven engineering trade-offs, the privacy and security model, and the evaluation. There is no user study, and the system has one long-term user.
...
**Statistical decoding of noisy input.** Modern mobile keyboards can treat a touch as evidence and decode a word or sentence against spatial and language models (Kristensson and Zhai, 2004; Vertanen et al., 2015). Alpha-OSK's mid-word prefix beam (section 3.1) belongs to this family: it searches live dictionary prefixes with a Gaussian emission over key positions. VelociTap jointly decodes sentence-length touchscreen input using character and word language models. Alpha-OSK instead offers incremental word candidates, with separate whole-word correction and heuristic source merging. It also learns mean offsets by physical key slot. Whether those offsets improve prediction for individual users is an empirical question: section 8.4 evaluates adaptation only in a synthetic biased-pointer simulation, not with real-user or longitudinal data.
...
Key identity alone discards the position of a press within a key. Continuous positions may help distinguish neighbouring intended keys. A stable directional offset is one possible error pattern, but neither its size nor its stability across users, layouts and sessions is established by this evaluation.

Two mechanisms follow. Each press reports its offset from the reported key's centre as a fraction of its width and height, and the prefix beam of §3.1 scores the continuous position rather than assuming the key's centre. Separately, `pointer_model.py` accumulates counts and summed offsets per physical slot (row and column, retained across a Dvorak or Colemak remap). It estimates each slot's mean offset, shrunk toward the global mean with ten pseudo-observations, and subtracts that estimate before scoring. These observations are relative to the reported key, not a known intended target. The table stores neither dispersion nor intended-key errors, so it is not a measure of pointer precision.
...
The table is owned by the n-gram predictor and uses its persistence, backup and Clear Learned Data paths. Privacy mode suppresses learning (§5.2), offsets are clamped to one key, and individual presses are not retained in this table. In the single seeded synthetic condition reported in §8.4, continuous positions add 0.5 percentage points of KSR and online mean-bias subtraction adds another 0.4. This demonstrates operation under that simulation; it does not establish a benefit from real-user personalisation.
...
**Protocol.** For each word in a held-out sentence, the simulated user requests the engine's top five predictions before each successive character, starting with the empty prefix, and accepts the intended word the first time it appears. A word accepted after *i* typed letters costs *i* + 1 clicks: the letters, plus one suggestion click. A word never predicted costs its full length plus one space, which is also the baseline. KSR is then
...
**Corpus.** The evaluation sets are the development and test splits of *A Crowdsourced Corpus of AAC-like Communications* (Vertanen and Kristensson, 2011), used under CC BY 4.0: 557 and 566 sentences, 2,956 and 2,730 words. Crowd workers invented communications as if using a scanning interface; these are AAC-like text proxies, not AAC users' typing logs. **The splits are by worker rather than by sentence**, so no author's writing appears in more than one split. The training split is absent from the repository. This is the 2011 COMM corpus, distinct from both COMM2 and Will Wade's generated `aac_datasetv2` correction dataset.

**Model state.** The tables record the historical configuration measured for this paper: 18,990 unigrams, 13,440 bigram prefixes over 67,697 edges, and 20,947 trigram prefixes from the shipped data files. Runs initialise in a fresh temporary directory and never use the developer's live learned model. Section 8.4 enables online pointer adaptation for one condition, and section 8.5 deliberately exposes the language model to additional text. These documentation revisions add no new measurements; the tables do not establish results for a later vocabulary or engine configuration.

**What this measures, and what it does not.** KSR assumes immediate recognition and error-free selection of every useful suggestion. In the noisy-input conditions, the harness supplies reference previous-word context and charges a missed word only its baseline length plus space. It neither propagates unresolved errors into later context nor charges repair actions. These conditions measure prediction-bar opportunities under a specified simulation, not final-text accuracy or net effort required to produce a correct sentence. Real users also pay visual-search, decision and movement costs. Section 9 returns to these limits.

**Separate outcomes.** Correction accuracy requires a declared unit and output policy, such as exact-sentence match or top-1 word recovery. WER and CER count substitutions, deletions and insertions divided by reference word or character count, with normalisation stated explicitly. Neither is KSR. Per-keystroke prediction latency is also distinct from whole-sentence correction latency and from task completion time. No WER/CER, comparative sentence-correction score or measured user-effort result is reported here. The proposed adapter, comparison rules and test cases are in [AAC benchmark evidence and plan](research/AAC_BENCHMARK_EVIDENCE.md).
...
The ideal selection policy avoids roughly half the baseline clicks and finds the intended next word before any of its letters about three times in ten. The two splits differ by 1.3 percentage points. That is descriptive evidence of corpus sensitivity, not a confidence interval or a statistical noise floor. It cannot establish that effects below 1.4 points are meaningless, or that larger effects are significant. The comparisons below report observed differences without inferential claims; future analyses should use paired comparisons with uncertainty estimated at the worker or conversation level.
...
|---|---|---|---|
| **Full system (measured snapshot)** | **49.1%** | **50.4%** | **2.9 ms** |
...
**The prefix beam improves on the previous whole-word fuzzy source in these runs.** Replacing it with that source lowers KSR by 1.7 points on dev and 1.2 on test. Whole-word correction used mid-word cannot reliably complete the intended word through an erroneous prefix. This comparison is against the previous source, not against disabling fuzzy predictions entirely; section 8.4 reports the synthetic slip condition.

**Including the fuzzy source does not improve clean-input KSR in these runs.** Removing it raises KSR by 0.3 points on dev and 0.1 on test. Those descriptive differences do not prove equivalence or that spatial correction is cost-free. The source also adds computation, as the latency column shows.

**Restoring PPM's word candidates lowers KSR and increases latency in these runs.** KSR changes by -0.6 and -0.5 points, while median per-keystroke latency rises from 2.9 ms to 15.9 ms, a factor of five and a half. The dictionary-free word path can emit fragments rather than complete words. PPM still trains and persists, but its word candidates no longer enter the merge. These ablations do not establish that character models cannot help a different decoder.
...
**The merge strategies have a small observed aggregate spread.** On `aac-test`, KSR is 50.4% for rank (the default), 50.5% for reciprocal rank fusion, 49.9% for linear, and 50.5% for log-linear. The 0.6-point range describes this corpus and configuration. Without a paired uncertainty analysis it supports neither a superiority claim nor an equivalence claim.

### 8.4 Robustness to pointer error

`--mis-click` injects one synthetic adjacent-key substitution on the second character of every word of four letters or more. The simulated user does not repair the slip. This is a controlled stress test of the suggestion bar, not an estimated error distribution for the target population.
...
This injection lowers KSR by 4.4 points on dev and 4.1 on test for the measured full system, versus 11.7 and 11.1 for the previous source. **Under this synthetic condition, the prefix beam improves KSR by 9.0 and 8.2 points over the previous whole-word fuzzy source.** That is not a comparison with the no-fuzzy condition and does not measure correction accuracy or real-user savings. The share of words never predicted rises from 14.6% to 29.8% on dev with the previous source, and from 13.6% to 16.2% with the beam.

**Online bias adaptation improves KSR in one seeded synthetic condition.** `--pointer` generates a landing point from the intended key's centre plus a fixed bias and Gaussian noise, chooses the nearest key centre as the reported key, and computes the offset from that reported key. Each comparison resets the bias table and uses seed 5. The learner sees reported keys and offsets during replay, without intended-key labels. On `aac-test`, the reported condition uses offsets of 0.35 and 0.25 key units and Gaussian noise with standard deviation 0.15 per axis:
...
Continuous click positions add 0.5 percentage points of KSR; online mean-bias subtraction adds 0.4, for 0.9 in total. An earlier per-key simulation reported intended-key recovery of 75% versus 86.5%, but that is a different outcome from prediction-bar KSR. Neither result establishes repeated-seed robustness, a confidence interval, later-session generalisation or benefit to an actual user. Runtime learning and its privacy/persistence paths are implemented and tested; their effectiveness as per-user adaptation remains unvalidated.

### 8.5 In-domain text exposure

`--learn-half` starts a fresh language model, trains on the first half of `aac-dev`, and evaluates the second half (279 sentences). This is an in-domain text-exposure experiment over crowd-authored sentences. It is not a chronological history from one user and does not evaluate longitudinal personalisation:
...
Training on the other half raises KSR by 2.2 points and next-word hit rate by 4.7 points in this split. The oracle row deliberately trains on the exact evaluation sentences: its 67.6% KSR illustrates the effect of test exposure. It is neither a perfect model of a user nor a bound on achievable personalisation. These rows do not isolate vocabulary from context effects or establish that federated learning would close the gap.

### 8.6 Latency

Median per-keystroke latency is 2.9 to 4.1 ms across the reported non-PPM conditions, with p95 between 10 and 16 ms, measured over `predict()` calls on the author's development machine. The budget in section 7 is 30 ms typical. Restoring PPM word candidates gives a 15.9 ms median on test and 24.5 ms on dev with a p95 of 44.3 ms, alongside the lower KSR reported in §8.3.

These are single-machine prediction-call measurements. They do not include full UI response, target-application insertion, sentence-correction latency, or users' decision and movement time, and they are not a performance specification for low-power devices. There is no cloud round-trip on this prediction path.
...
**The evaluation corpus is a proxy.** The AAC corpus is crowd-written by workers imagining a scanning interface, not collected from people using this or any other keyboard. Its worker-disjoint splits support evaluation on unseen authors within that collection, but invented AAC-like sentences need not match actual correspondence, code or medical communication. The pointer conditions add synthetic errors to these texts; there are no recorded click trajectories or longitudinal per-user adaptation results. Will Wade's `aac_datasetv2` adds another synthetic text-error source, not the missing user or pointer evidence. Its provenance, split-leakage risks and limits are documented in [AAC benchmark evidence and plan](research/AAC_BENCHMARK_EVIDENCE.md).
...
4. **Seeding the context tables from a larger corpus.** The generated layer added in 1.3.0 raises KSR by 1.8 and 1.7 points and next-word hit rate by 5.8 and 5.2 points in the reported comparisons (§8.3). Increasing the trigram prune from 20,000 to 50,000 contexts added another 0.3 points on test for 2.5 times the file size and 315 ms more startup. That is an observed quality/resource trade-off, not a statistically established negligible effect.
