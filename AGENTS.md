# AGENTS.md: Alpha-OSK AI Onboarding
...
- **Settings** (layout, theme, toggles): Managed by Qt `Settings` in QML. Auto-saved on change. Stored in OS registry/config automatically by Qt: on Windows that is `HKCU\Software\alpha-osk`, the **organisation** name from `keyboard_app.py::app.setOrganizationName`.
  - **The Windows uninstaller must not delete that key except behind its own prompt.** Releases through 1.3.0 deleted `Software\${APP_NAME}` unconditionally. Registry keys are case-insensitive, so that erased the entire Qt organisation tree whenever an upgrade ran the old uninstaller. Since 1.4.0 deletion lives in `installer.nsh::customUnInstall`, behind `IfSilent` and the data-removal prompt, using `APP_ORG` (which must match `setOrganizationName`). That alone did not protect the upgrade delivering the fix. **Every invocation of a previous uninstaller now goes through `upgrade_settings.nsh::RunPreviousUninstaller`.** It copies the complete settings tree, preserving registry types, to a unique sibling under `HKCU\Software\alpha-osk-upgrade-backups`, flushes it, waits for the actual old uninstaller using `/S _?=<directory>`, then restores settings before continuing. Backup failure aborts before cleanup; restore failure aborts and retains the recovery copy, whose path is reported by setup. A successful restore removes its backup, even if the old process returned an error (which still aborts setup). No editable registry export is imported by the elevated installer. Structural coverage is in `tests/test_windows_installer.py`; executable Windows/NSIS regression coverage is in `tests/test_upgrade_settings.py`. Previously deleted preferences cannot be reconstructed by this protection.
