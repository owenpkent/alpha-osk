...
Popup {
    ...
    // Whether Shift is held, so an arrow extends the selection instead
    // of moving the caret. Bound from Main.qml rather than read off the
    // bridge here, like every other dependency this component declares.
    ...
    // Shift turns a caret move into a selection, the way it does in a
    // real text box. Same helper the snippets editor carries, and there
    // for the same reason: this window never holds OS focus, so Qt's own
    // key handling never sees the modifier.
    function _moveCaret(f, to) {
        if (editor.shiftOn) f.moveCursorSelection(to)
    ...
    Connections {
        ...
        function onEditSpecialPressed(name) {
            ...
            if (name === "backspace") {
                ...
                editor._moveCaret(f, Math.max(0, pos - 1))
            } else if (name === "right") {
                editor._moveCaret(f, Math.min(len, pos + 1))
            } else if (name === "home") {
                editor._moveCaret(f, 0)
            } else if (name === "end") {
                editor._moveCaret(f, len)
            } else if (name === "space") {
                if (f.selectedText) f.remove(f.selectionStart, f.selectionEnd)
