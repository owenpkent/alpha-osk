...
- [ ] **The Filter Explicit Words toggle cannot tell you when it is governing nothing.** `HybridPredictor.explicit_filter_available` reports whether `data/explicit_words.txt` loaded, and it exists precisely because `_load_explicit_words` fails open: an unreadable file must not stop the keyboard starting, so the filter goes inert instead. Nothing outside the predictor reads that property. There is no bridge slot and the toggle has no unavailable state, so a missing list leaves the setting reading as on while it suppresses nothing, and the only trace is a line in the diagnostic log. `passwordDetectionAvailable` is the pattern to copy: a read-only property on the bridge, an amber note in *Settings -> Smart Typing -> Suggestions*, and the toggle disabled. Found in the 1.5.0 pre-release review; the docstring in `hybrid_predictor.py` had claimed a `getExplicitFilterAvailable` slot that was never written, and now says so instead.

- [ ] **Cold start costs about 3 s against 0.8 s before the expanded vocabulary**, paid on every launch before the window can appear, on a device whose user has no other way to type. Warm construction is fine (the process-level caches took `NgramPredictor()` to 43 ms and the suite below `main`'s time), but the first predictor in a process still builds both packed indexes from scratch: roughly 4.3 million deletion variants for the deletion index and the prefix index on top. The caches cannot help the first one by construction. What would: building the packed buffers offline and shipping them as a data file, so a launch maps them rather than computes them, with the validation and a version check on load. That is a real piece of work and it is the main measured cost of 1.5.0, recorded here rather than left to be rediscovered as "the keyboard got slower to start".

- [ ] **`scripts/clean_branches.py` crashes on a merged branch that another worktree has checked out.** Seen on 2026-09-13: `fix/cf-worker-sharp-libheif` (PR #112, merged) was still checked out in a Claude session's worktree under `.claude/worktrees/`, so `git branch -D` refused it, and the script's `subprocess.run(..., check=True)` raised `CalledProcessError` out of `main()`. Branches earlier in the list had already been deleted; any later in the list would not have been. Under the `post-merge` hook the failure is silent, because the hook ends in `|| true`, so the only symptom is merged branches quietly staying behind. The fix is to treat a branch checked out in any worktree (`git worktree list --porcelain`) as kept, with the reason printed, the same way the script already refuses the current branch, and to catch a failed delete per branch rather than aborting the run. Do not remove the other worktree to make the delete succeed: it may belong to a session that is still working in it.

- [ ] **Two tests in `tests/test_fuzzy_prefix_beam.py` are order-dependent.** `TestATwoLetterMisClickDoesNotEmptyTheBar::test_a_single_character_is_still_too_little_to_act_on` and `test_a_live_two_letter_prefix_is_still_the_exact_completers_alone` read `word_generator._prefix_beam` straight off the module-scoped recognizer, but the beam is built lazily on the first completion. They pass only when an earlier test in the same process has already asked for one, so each fails on its own and whenever xdist schedules it apart from that test. Fixed on the PR #114 branch (commit `70ac8d1`, a helper that builds the beam before reading it); tick this when that PR lands.
