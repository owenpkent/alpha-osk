...
- **Digits come back via a panel, not a fifth row and not a toggle.**
  `Main.qml::showNumberRow` is derived: true exactly when the active layout JSON
  carries no `number` row of its own, which is the compact variants and nothing
  else. Every key must register through `registerFn` or the swipe overlay
  swallows every tap on it, and the panel's leading key is **Esc, not `` ` ``**,
  registered as a `special` rather than a `char` so it stays hit-testable without
  putting a phantom "Esc" centre into the swipe shape match (see the two-registry
