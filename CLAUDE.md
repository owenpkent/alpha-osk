...
**The emission takes a position.** `get_fuzzy_predictions(..., positions=[...])` carries one optional `(row, col)` in key units per character of the current word, defaulting to the key's centre, so forwarding `mouse.x / mouse.y` from `KeyButton.qml` (captured today for the ripple and discarded) is plumbing on top of this, not a rewrite of the scoring.
