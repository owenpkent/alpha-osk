...
The installer's participation page (`build/windows/build.py`, `Page custom StudyInvitePage`) invites the user to share the ten anonymous counters. Four things about it are load-bearing:

- **The checkbox ships UNCHECKED and must stay that way.** A pre-ticked consent box is not consent, and it is the single thing that would make the data unusable as research and non-compliant as a privacy control.
