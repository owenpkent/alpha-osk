...
**A live snippet offer survives this reset alone.** `_check_external_click` passes `keep_snippet_offer=True`; every other caller takes the default, and privacy mode in particular must. The offer describes a value the user typed rather than a caret position, and clicking the next field of the same form is the single most likely thing to happen right after an email address is typed, so withdrawing here closed the Save button before the user could travel to it. An app switch still withdraws it through `_check_foreground_window`'s own reset.
