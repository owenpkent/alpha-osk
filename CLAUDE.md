...
**Moving follows the snippet, not the slot** (`moveSnippet` updates `menuIndex` and the page it landed on). The sheet is about one snippet, and staying on the index would silently retarget it at whichever one swapped in.
