...
**CI-time CVE scanning.** `.github/workflows/ci.yml` has an `osv-scan` job pinned to `google/osv-scanner-action@9a498708959aeaef5ef730655706c5a1df1edbc2` (v2.3.8) that reads both lockfiles (`requirements-dev.txt` + `backend/cf-worker/package-lock.json`) and queries the OSV database on every push/PR. **Currently advisory only** (`fail-on-vuln: false`) because we already know about four dev-only CVEs flowing through Wrangler 3.x (3 moderate esbuild, 1 high undici — upgrading is a breaking change for the worker, tracked separately). Findings still appear in the Security tab via SARIF upload. Flip `fail-on-vuln: true` once the known noise is resolved or quarantined, so unknowns gate merges.
